from flask_cors import CORS
from flask import Flask, request, jsonify, render_template
from PIL import Image, ImageTk
import numpy
from tensorflow.keras.models import load_model
import math
from skimage import io, transform

app = Flask(__name__)

model = load_model('traffic_classifier.h5')

# allow cors in flask
CORS(app)


def prepare_response(label, p, error_text='An Error Occured'):
    if label is not None:

        return {
            'class': str(label),
            'class_desc': classes[label],
            'confidence': math.floor(p * 100)
        }
    elif label is None:
        return {
            'class': 'None',
            'class_desc': 'Traffic Sign not present',
            'confidence': math.floor(classes[0][numpy.argmax(classes)] * 100)
        }
    else:
        return {
            'class': 'Error',
            'class_desc': error_text,
            'confidence': ''
        }


# dictionary to label all traffic signs class.
classes = {
    1: 'Speed limit (20km/h)',
    2: 'Speed limit (30km/h)',
    3: 'Speed limit (50km/h)',
    4: 'Speed limit (60km/h)',
    5: 'Speed limit (70km/h)',
    6: 'Speed limit (80km/h)',
    7: 'End of speed limit (80km/h)',
    8: 'Speed limit (100km/h)',
    9: 'Speed limit (120km/h)',
    10: 'No passing',
    11: 'No passing veh over 3.5 tons',
    12: 'Right-of-way at intersection',
    13: 'Priority road',
    14: 'Yield',
    15: 'Stop',
    16: 'No vehicles',
    17: 'Veh > 3.5 tons prohibited',
    18: 'No entry',
    19: 'General caution',
    20: 'Dangerous curve left',
    21: 'Dangerous curve right',
    22: 'Double curve',
    23: 'Bumpy road',
    24: 'Slippery road',
    25: 'Road narrows on the right',
    26: 'Road work',
    27: 'Traffic signals',
    28: 'Pedestrians',
    29: 'Children crossing',
    30: 'Bicycles crossing',
    31: 'Beware of ice/snow',
    32: 'Wild animals crossing',
    33: 'End speed + passing limits',
    34: 'Turn right ahead',
    35: 'Turn left ahead',
    36: 'Ahead only',
    37: 'Go straight or right',
    38: 'Go straight or left',
    39: 'Keep right',
    40: 'Keep left',
    41: 'Roundabout mandatory',
    42: 'End of no passing',
    43: 'End no passing veh > 3.5 tons'
}


@app.route('/upload', methods=['POST'])
def upload():
    if request.method == 'POST':
        if 'file' not in request.files:
            # print('NO FILE FOUND!')
            return jsonify(prepare_response(None, error_text='Image Upload Error!'))
        file = request.files['file']
        if file.filename == '':
            # print('NO FILE SELECTED')
            return jsonify(prepare_response(None, error_text='File Invalid'))
        if file:
            image = request
            global label_packed

            # image = io.imread(file)
            # image = transform.resize(image, (30, 30))
            image = Image.open(file)
            image = image.resize((30, 30))
            image = numpy.expand_dims(image, axis=0)
            image = numpy.array(image)
            print(image.shape)

            pred = [(i, j) for i, j in enumerate(
                model.predict([image])[0]) if j > 0.5][0]

            label = pred[0]+1
            confidence = pred[1]
            print(label)
            sign = classes[label]
            print(sign)

            return jsonify(prepare_response(label, confidence))
    else:
        return jsonify(prepare_response(None, error_text='Invalid Request'))


@app.route('/')
def home():
    return render_template('index.html')


if __name__ == "__main__":
    app.run(debug=True)
